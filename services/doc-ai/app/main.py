...
SECTION_PATTERNS = {
    ...
    "company_opinion": [
        r"민원인의\s*주장\s*및\s*요구사항에\s*대한\s*당사의\s*구체적\s*의견",
        r"민원의\s*주장\s*및\s*요구사항에\s*대한\s*당사의\s*구체적\s*의견",
        r"당사.*구체적\s*의견",
    ],
    "violation_and_action": [r"임직원\s*위규.*조치\s*내용", r"위규.*조치\s*내용"],
    "future_action_plan": [
        r"민원인의\s*요청사항에\s*대한\s*당사의\s*의견\s*및\s*향후\s*처리\s*방안",
        r"향후\s*처리\s*방안",
    ],
}

# 「향후 처리방안」 등 본문 이후 행정·서식 문단(7~8절, 심의 문구) — 버퍼에 넣지 않음
_FUTURE_SECTION_STOP_LINE = re.compile(
    r"^(?:"
    r"\s*(?:7|７)\.\s*검토의견\s*등에\s*대한\s*문의처|"
    r"\s*(?:8|８)\.\s*기타\s*동\s*민원\s*관련\s*참고사항|"
    r"\s*※\s*이\s*회신문의\s*내용은\s*소비자보호부서의\s*최종\s*심의를\s*거쳐|"
    r"\s*※\s*이\s*회신문의\s*내용은|"
    r"위\s*내용과\s*관련하여\s*추가\s*문의\s*사항이\s*있으시면"
    r")",
    re.MULTILINE,
)


def _trim_future_action_tail(text: str) -> str:
    """추가 방어: 본문에 섞여 들어간 7~8절·※ 문구 이후 절단."""
    if not text.strip():
        return text
    lines = text.split("\n")
    out: list[str] = []
    for line in lines:
        s = line.strip()
        if _FUTURE_SECTION_STOP_LINE.search(s):
            break
        out.append(line)
    return "\n".join(out).strip()
...
def _extract_sections(paragraphs: list[str]) -> dict[str, str]:
    ...
    for paragraph in paragraphs:
        ...
        if not line:
            continue
        # 향후 처리방안 본문 끝(문의처·참고·심의 문구 시작)에서는 여기서 끊음
        if current_key == "future_action_plan" and _FUTURE_SECTION_STOP_LINE.search(line):
            flush()
            current_key = None
            continue
        matched_key = None
    ...
    flush()
    if result.get("future_action_plan"):
        result["future_action_plan"] = _trim_future_action_tail(str(result["future_action_plan"]))
    return result
